...
const char output_file[] = "/home/boxiang-vm/Documents/Research/small_graph/dataset_sssp/sssp_cpu_stl.result";
const char debug_file[] = "/home/boxiang-vm/Documents/Research/small_graph/dataset_sssp/sssp_cpu_stl.debug";
...
void sssp_stl(index_t root, graph *g)
{
...
void sssp_dijkstra(index_t root, graph *g)
{
    priority_queue<index_t> q;
    for (index_t i = 0; i < g->vert_count; ++i)
        sa[i] = INF;
    q.push(root);
    sa[root] = 0;
...
void sssp_bellman(index_t root, graph *g)
{
    for (index_t i = 0; i < g->vert_count; ++i)
        sa[i] = INF;
    sa[root] = 0;
    for (index_t i = 0; i < g->vert_count - 1; ++i) // outer loop
    {
        for (index_t j = 0; j < g->vert_count; ++j) // for each edge
        {
            for (index_t k = g->beg_pos[j]; k < g->beg_pos[j + 1]; ++k)
            {
                index_t v = g->csr[k]; // destination node
                if (sa[v] > sa[j] + g->weight[k])
                {
                    sa[v] = sa[j] + g->weight[k];
                }
            }
        }
    }
...
int main(int args, char **argv)
{
    ...
    sssp_bellman(0, g); // g->vert_count, g->edge_count);
